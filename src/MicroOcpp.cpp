...
std::shared_ptr<Transaction> beginTransaction_authorized(const char *idTag, const char *parentIdTag, unsigned int connectorId) {
...
#if MO_ENABLE_V201
    // Fix: en OCPP 2.0.1 les connecteurs sont des EVSE gérés par TransactionService.
    // On utilise beginAuthorization(idToken, validateIdToken=false) pour bypasser
    // l'autorisation RFID, équivalent v201 de beginTransaction_authorized().
    if (context->getVersion().major == 2) {
        ...
        if (!evse) {
            MO_DBG_ERR("could not find EVSE %u", connectorId);
            return nullptr;
        }
        // Si une transaction résiduelle est encore active (ex: reboot sans arrêt propre),
        // on la termine d'abord pour libérer l'EVSE.
        auto *existingTx = evse->getTransaction();
        if (existingTx && existingTx->isAuthorizationActive) {
            MO_DBG_WARN("EVSE %u: clearing residual transaction before new beginAuthorization", connectorId);
            evse->abortTransaction(
                Ocpp201::Transaction::StoppedReason::PowerLoss,
                Ocpp201::TransactionEventTriggerReason::AbnormalCondition);
            // abortTransaction met active=false mais isAuthorizationActive reste true
            // jusqu'au prochain mocpp_loop(). On le force ici pour débloquer beginAuthorization.
            existingTx->isAuthorizationActive = false;
        }
        // Type::NoAuthorization = token OCPP 2.0.1 pour "pas de badge RFID".
        // CitrineOS répond Accepted sans chercher le token dans sa base.
        IdToken idToken (idTag, IdToken::Type::NoAuthorization);
        if (!evse->beginAuthorization(idToken, /*validateIdToken=*/false)) {
            MO_DBG_ERR("beginAuthorization failed for EVSE %u", connectorId);
            return nullptr;
        }
        // En v201, getTransaction() retourne Ocpp201::Transaction* (pas un shared_ptr).
        // On retourne nullptr : le appelant doit utiliser isTransactionActive() pour
        // vérifier le succès, comme le font toutes les autres fonctions v201.
        return nullptr;
    }
